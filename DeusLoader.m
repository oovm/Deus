...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated Deus loader file *)


Unprotect[Deus`Private`Package`$TopLevelLoad];
Deus`Private`Package`$TopLevelLoad=
...
PackageFileContext[f_String?DirectoryQ]:=
	With[{s=PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s,Deus],"`"]
		];
...
PackageScopeBlock[e_,scope_String:"Hidden"]/;TrueQ[$AllowPackageRescoping]:=
	With[{newcont="Deus`Private`"<>StringTrim[scope,"`"]<>"`"},
...
PackageDecontext[
	...
	With[{
		...
		ctx="Deus`Private`"<>StringTrim[scope,"`"]<>"`"
		},
		Replace[names,
...
If[(Clear@Deus`Private`Package`$loadAbort;!#)&@Deus`Private`Package`$loadAbort,
	Deus`Private`Package`PackagePostProcessContextPathReassign[]
	]